...
class RegistroForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if telefono and not re.fullmatch(r'3[0-9]{9}', telefono):
            # Este error de validación del campo 'telefono' también aparecerá debajo del campo.
            raise ValidationError('El número debe ser colombiano: +57 seguido de 10 dígitos (ej: 3001234567).')
...
class CambiarPasswordForm(forms.Form):
    password_old = forms.CharField(label="Contraseña actual", widget=forms.PasswordInput(attrs={"class": "input"}))
    password_new = forms.CharField(label="Nueva contraseña", widget=forms.PasswordInput(attrs={"class": "input"}))
    password_confirm = forms.CharField(label="Confirmar contraseña", widget=forms.PasswordInput(attrs={"class": "input"}))